...
// RegisterCloseable registers a pre-built value for type T together with a disposer that [Container.Close] runs to release it. The value is returned as-is on every [Resolve].
//
// Unlike [Register], the container owns this value's cleanup. Re-registering the same key records an additional disposer, so a value replaced by a later registration is still closed.
...
// RegisterSingletonCloseable registers a singleton factory for type T together with a disposer. The factory is invoked once on first [Resolve]; at that point the disposer is recorded and later run by [Container.Close] in reverse order of construction. An unresolved singleton constructs nothing and records no disposer, so nothing is closed for it.
