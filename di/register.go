...
// WithName qualifies a registration or lookup with a name, so that multiple
// values of the same type can coexist in one container. When omitted, a value
// is keyed by its type alone.
...
// RegisterTransient registers a factory for type T that is invoked fresh on
// every [Resolve]; the result is never cached. The factory resolves its own
// dependencies from c using the passed [context.Context].
