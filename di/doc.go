...
//
// Dependencies are keyed by their concrete Go type, optionally qualified by a name so that multiple values of the same type can coexist. The public API is generic and typed end-to-end — there is no untyped registration or lookup:
//
//	c := di.NewContainer()
...
//
// Constructor injection is the only wiring pattern: a factory receives the resolution [context.Context] and calls [Resolve] with it for each dependency it needs. The active resolution chain travels in that context, so circular dependencies are detected and reported as an error, and a canceled context aborts resolution.
//
// Resource cleanup is opt-in: only values registered with [RegisterCloseable] or [RegisterSingletonCloseable] are released by [Container.Close], which runs their disposers in reverse order of construction. Plain [Register] values and unresolved singletons are never closed by the container — the caller owns them.
package di
