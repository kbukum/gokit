...
//
// Dependencies are keyed by their concrete Go type, optionally qualified by a name
// so that multiple values of the same type can coexist. The public API is generic
...
// and reported as an error, and a canceled context aborts resolution.
//
...
// which runs their disposers in reverse order of construction. Plain [Register] values
// and unresolved singletons are never closed by the container — the caller owns them.
package di
