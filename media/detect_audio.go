...
func detectAudio(data []byte) (Info, bool) {
	...
	if len(data) >= 12 && string(data[0:4]) == "RIFF" && string(data[8:12]) == "WAVE" {
		return Info{Type: Audio, Format: FormatWAV, MimeType: "audio/wav", Container: "RIFF"}, true
	}

	// FLAC — "fLaC" magic.
	if data[0] == 'f' && data[1] == 'L' && data[2] == 'a' && data[3] == 'C' {
		return Info{Type: Audio, Format: FormatFLAC, MimeType: "audio/flac"}, true
	}
	...
	if data[0] == 'M' && data[1] == 'T' && data[2] == 'h' && data[3] == 'd' {
		return Info{Type: Audio, Format: FormatMIDI, MimeType: "audio/midi"}, true
	}

	// AIFF — "FORM....AIFF"
	if len(data) >= 12 && string(data[0:4]) == "FORM" && string(data[8:12]) == "AIFF" {
		return Info{Type: Audio, Format: FormatAIFF, MimeType: "audio/aiff", Container: "IFF"}, true
	}
