...
// ftypBrand returns the 4-character major brand (bytes 8..12) of an ISO BMFF "ftyp" header when data begins with a well-formed ftyp box.
// The single length guard keeps every slice access provably in bounds.
...
// It fills the detection window even when r yields the data in small chunks,
// so multi-byte signatures (RIFF, ISO BMFF ftyp, MPEG-TS) are not missed by a partial read.
// A stream shorter than the window is detected on what it holds; an empty stream returns an error.
...
// isText returns true if data appears to be UTF-8 text. It checks that the data is valid UTF-8
// and has a high ratio of printable characters (letters, digits, punctuation, whitespace).
