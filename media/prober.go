...
// ErrUnsupported is returned by a [Prober] when it does not recognize the
// content it was given. It lets a [Registry] skip a prober and try the next one.
var ErrUnsupported = errors.New("media: unsupported content for prober")

// Metadata is the enriched result of probing content: the detection [Info] plus
// any lightweight properties a backend could extract without full processing.
//
// It is the light-kit parallel of rskit's media metadata; the light surface
// carries only cheaply derivable fields (currently image/frame dimensions).
type Metadata struct {
	Info
	// Resolution is the pixel dimensions for images or video frames, or the
	// zero [Resolution] when the probing backend cannot determine them.
