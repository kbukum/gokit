...
type Config[V any] struct {
	// TTL is the time-to-live before expiration. Zero means never expire. If KeepAlive is true,
	// TTL resets on each Append/Touch. If KeepAlive is false,
	// TTL is absolute from creation/last flush.
	TTL time.Duration

	// KeepAlive enables sliding window TTL. When true, TTL resets on each Append or Touch operation.
	// When false, TTL is absolute.
	KeepAlive bool

	// MinSize is the minimum size (measured by Measurer) to trigger flush. Zero means no minimum.
	// Used by SizeTrigger.
	MinSize int
	...
	// MinInterval is the minimum time between flushes.
	// Prevents too-frequent processing even if triggers fire. Zero means no rate limiting.
	...
	// TriggerMode defines how multiple triggers are combined. TriggerAny (default):
	// flush if ANY trigger fires (OR logic) TriggerAll: flush only if ALL triggers fire (AND logic)
	TriggerMode TriggerMode

	// OnFlush is called when the accumulator flushes values. Receives the flushed values
	// and should return nil on success.
	...
	// OnError is called when an error occurs during background operations (trigger checks, expiration checks).
	// Optional - can be nil. If nil, errors are silently ignored.
