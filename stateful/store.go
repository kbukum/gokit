...
// Store provides typed storage for accumulation. Implementations can use any backend: memory,
// Redis, Postgres, DynamoDB, filesystem, etc.
//
...
type Store[V any] interface {
	...
	// Returns the evicted values (if any) and an error if operation fails. If maxSize is 0
	// or negative, behaves like Append (no eviction).
	...
	// LastActivity returns the time of last activity (append or touch). Used to check expiration.
	// Returns zero time if never active.
