...
// The first in the list is the outermost (runs first on a request, last on a response).
...
// Use this when you need to apply a Middleware directly on the Gin engine instead of at the server handler level.
//
// Note:
// middleware that wraps http.ResponseWriter (e.g. RequestLogger) may not fully integrate with gin.Context.Writer.
// Prefer applying such middleware at the server level via ApplyMiddleware().
