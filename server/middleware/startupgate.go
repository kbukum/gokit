...
// StartupGate blocks API requests with 503 Service Unavailable until MarkReady is called. Infrastructure paths (/health, /ready, /alive, /info, /version, /metrics) are always allowed through so orchestrators can probe the service during startup.
...
// NewStartupGate creates a gate in the "not ready" state. By default, /health, /ready, /alive, /info, /version, and /metrics are always allowed through.
...
// MarkReady signals that the service is fully initialized and API traffic should be allowed through.
