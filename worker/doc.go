// Package worker provides push-based task execution with real-time event streaming, worker pools,
// and supervision.
//
// The core abstraction is a Handler — a function that receives typed input, does work,
...
// # Pool
//
// Pool manages N goroutines running the same handler with dispatch strategies, event aggregation,
...
// # Middleware
//
// Middleware[I, O] wraps a Handler with cross-cutting behavior.
// Chain composes multiple middlewares (same pattern as provider.Middleware):
//
...
// # Composition
//
// Handlers compose via FanOut (same input to N handlers), NewMapReduce (split → process → combine),
// and NewPipeline (sequential chaining).
//
