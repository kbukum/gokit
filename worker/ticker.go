...
// WithRunOnStart causes the worker to execute fn once immediately when the background goroutine starts,
// before entering the periodic loop. The initial run does NOT block app startup —
// it runs inside the goroutine launched by Start.
func WithRunOnStart() TickerOption {
	return func(w *TickerWorker) { w.runOnStart = true }
}

// WithOnError registers a callback that is invoked after every tick that returns a non-nil error.
...
//
// Start launches a background goroutine; Stop signals it and waits for a clean exit.
// Health reports the last-run time and any recent errors.
//
// Example:
//
