...
// FanOut sends the same input to N handlers concurrently.
// Returns when all complete. Events from all handlers are merged into the
// composite emit. Results are collected in the same order as handlers.
...
// NewMapReduce creates a handler that splits input, processes sub-tasks
// concurrently via Handler, and combines results.
// If cfg.Pool is set, it is reused across invocations (caller manages its lifecycle).
...
//
// Due to Go's generics limitations, pipeline stages use any internally
// with runtime type assertions. For compile-time safety, compose handlers
// manually or use dag with typed ports.
