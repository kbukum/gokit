...
//
// `ready` is closed exactly once when either `handle` is published or `err` is set. The close acts as the happens-before barrier for both fields, so they may be read without holding KeyedPool.mu after `<-ready` returns.
//
// `cancelSubmit` cancels the context passed to pool.Submit. Calling it during phase 1 unblocks Submit with ctx.Err(); calling it later is a no-op. It is held alongside (not instead of) the eventual handle's Cancel so a single Cancel call works in every phase.
...
func (k *KeyedPool[K, I, O]) SubmitOrAttach(ctx context.Context, key K, task I) (*TaskHandle[O], bool, error) {
	...
	// Reserve under the lock so concurrent same-key callers attach instead of double-submitting. We derive a cancellable ctx so Cancel(key) can interrupt Submit during phase 1.
	...
	if submitErr != nil {
		// Phase 1 failure: clear the entry first, then publish the error. New callers post-clear see no entry; callers that captured the entry pointer pre-clear wait on ready and observe the error.
		k.mu.Lock()
	...
	// Auto-evict on completion. Spawned once per real submission; the goroutine cost is the price of map cleanliness. cancelSubmit is invoked here too so the derived ctx is released even on natural completion.
	go func() {
		<-h.Done()
		k.mu.Lock()
		if cur, ok := k.inflight[key]; ok && cur == entry {
...
func (k *KeyedPool[K, I, O]) Cancel(key K) bool {
	...
	entry.cancelSubmit()
	// If the handle is already published, cancel it directly. select-default keeps Cancel non-blocking during phase 1; the cancelSubmit call above is sufficient to terminate that phase.
	select {
	case <-entry.ready:
		if entry.handle != nil {
			entry.handle.Cancel()
