...
// DefaultBroadcastBuffer is the per-subscriber buffer used when no buffer size is configured.
// A subscriber lagging more than this many unconsumed events drops the overflow rather than stalling the broadcaster.
const DefaultBroadcastBuffer = 64

// Broadcaster is a bounded, cancellable fan-out source:
// it turns "observe a backend" into a bounded stream of typed events delivered to many independent subscribers.
...
// Without WithBroadcastBuffer it uses DefaultBroadcastBuffer.
...
// ensureInit lazily initializes the fields a zero-value Broadcaster leaves unset,
// so it is safe to use without NewBroadcaster (mirroring the zero-value readiness of sync primitives).
...
// Subscribe registers a new subscriber and returns its receive-only event channel.
// The channel is closed — terminating any range over it — when ctx is canceled
// or the Broadcaster is closed. Subscribing with an already-canceled context,
// or to a closed Broadcaster, returns an already-closed channel without registering a subscriber
// or spawning a watcher goroutine.
...
// and all subscriber goroutines are released. It is idempotent,
// and further Broadcast calls become no-ops.
...
// remove unregisters sub and closes its channel exactly once. Broadcast and remove both hold b.mu,
// so a channel is never sent to after it is closed.
