...
// DefaultBroadcastBuffer is the per-subscriber buffer used when no buffer size
// is configured. A subscriber lagging more than this many unconsumed events
// drops the overflow rather than stalling the broadcaster.
const DefaultBroadcastBuffer = 64

// Broadcaster is a bounded, cancellable fan-out source: it turns "observe a
// backend" into a bounded stream of typed events delivered to many independent
// subscribers. Each subscriber owns a private bounded channel — a subscriber
// that falls further behind than the buffer loses interim events (backpressure
// by drop) but never blocks the broadcaster or its peers. This is the canonical
// owner for the "watch a source → typed change stream" shape that recurs across
...
// WithBroadcastBuffer sets the per-subscriber buffer size. Values below 1 are
// clamped to 1 so every subscriber can hold at least one in-flight event.
func WithBroadcastBuffer(size int) BroadcasterOption {
	return func(c *broadcasterConfig) { c.buffer = size }
}

// NewBroadcaster creates a Broadcaster with the given options. Without
// WithBroadcastBuffer it uses DefaultBroadcastBuffer.
...
// Broadcast delivers item to every live subscriber. Delivery to a full
// subscriber is dropped rather than blocked, so a slow subscriber never stalls
// the broadcaster or its peers. Broadcasting after Close is a no-op.
...
// Close terminates the Broadcaster: every subscriber channel is closed and all
// subscriber goroutines are released. It is idempotent, and further Broadcast
// calls become no-ops.
...
// remove unregisters sub and closes its channel exactly once. Broadcast and
// remove both hold b.mu, so a channel is never sent to after it is closed.
