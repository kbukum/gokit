// Package stream provides composable, pull-based data stream operators plus a
// bounded push fan-out source.
//
// # Canonical shape
//
// gokit converges on the rskit-stream operator vocabulary
// (map/filter/fan_out/window/batch/parallel/merge/partition/…) but keeps an
// idiomatic Go pull-iterator model for transformation pipelines: pipelines are
// lazy — no work happens until values are pulled via Collect, Drain, or ForEach,
...
//
// The genuinely push-shaped concern — fanning one source out to many
// independent observers — is served by Broadcaster, mirroring rskit's
// Broadcaster<T>. Every subscriber owns a private bounded channel; a subscriber
// that lags beyond its buffer drops overflow (backpressure by drop) but never
// blocks the broadcaster or its peers.
//
// # Bounded buffers
//
// No operator buffers without bound. Buffer clamps size <= 0 to 1; Broadcaster
// clamps its per-subscriber buffer to at least 1; the time/size-aware operators
// (Batch, TumblingWindow, SlidingWindow) emit and release each group as it
// completes. Concurrent operators (Parallel, Merge, Buffer) run owned goroutines
// bounded by ctx cancellation and closed via the iterator's Close.
//
