...
func TestRetryHandler_SetsRetryCountHeader(t *testing.T) {
	...
	wrapped := RetryHandler(handler, cfg)
	msg := kafka.Message{Topic: "t", Headers: map[string]string{"existing": "value"}}
...
func TestRetryHandler_OnExhaustedCalled(t *testing.T) {
	...
	var exhaustedMsg kafka.Message
	...
	cfg := RetryMiddlewareConfig{
		...
		OnExhausted: func(_ context.Context, msg kafka.Message, err error) {
			exhaustedMsg = msg
			exhaustedErr = err
		},
	}
	...
	wrapped := RetryHandler(handler, cfg)
	msg := kafka.Message{Topic: "orders", Key: "k1", Headers: map[string]string{}}
	_ = wrapped(context.Background(), msg)
...
func TestRetryHandler_OnExhaustedNotCalledOnSuccess(t *testing.T) {
	...
	cfg := RetryMiddlewareConfig{
		...
		OnExhausted: func(_ context.Context, _ kafka.Message, _ error) {
			called = true
		},
	}

	wrapped := RetryHandler(handler, cfg)
	_ = wrapped(context.Background(), kafka.Message{Headers: map[string]string{}})
...
func TestRetryHandler_NilHeaders(t *testing.T) {
	...
	err := wrapped(context.Background(), kafka.Message{Topic: "t"})
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
}
...
func TestRetryHandler_DoesNotMutateOriginalHeaders(t *testing.T) {
	...
	msg := kafka.Message{Topic: "t", Headers: original}
