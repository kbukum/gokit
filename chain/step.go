...
// newStepContext builds a StepContext bound to ctx with an optional
// step-local progress reporter.
...
// Progress reports step-local progress. percent is clamped to 0..=100 and
// message may be empty. It is a no-op when no progress reporter is attached.
...
// StepFn executes a single typed step. It receives the previous step's output
// (or the chain input for the first step) and returns this step's output.
type StepFn[I, O any] func(sctx StepContext, input I) (O, error)

// CleanupFn releases resources produced by a completed step. It runs only when
// a later step fails or the chain is canceled, in reverse (LIFO) order.
type CleanupFn func(ctx context.Context) error

// Step is a typed operation in a sequential chain: it consumes an I and
// produces an O. Steps are values; construct them with NewStep or StepFunc and
// register cleanup with WithCleanup.
...
// or the chain is canceled after this step has completed. It returns a copy of
// the step; the receiver is not modified.
