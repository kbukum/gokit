...
func TestEmail_TableDriven(t *testing.T) {
	...
	tests := []struct {
		...
		{"missing dot in domain", "user@example", true},
...
func TestURL_TableDriven(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name    string
		value   string
		...
		{"valid http with path", "http://example.com/path?q=1", false},
		{"empty value", "", true},
		{"missing scheme", "example.com", true},
		{"missing host", "https:///path-only", true},
...
func TestBefore_TableDriven(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			v := New()
			v.Before("start", tt.value, deadline)
			if got := v.HasErrors(); got != tt.wantErr {
				t.Errorf("Before(%s, %s): HasErrors() = %v, want %v",
					tt.value.Format(time.RFC3339Nano), deadline.Format(time.RFC3339Nano), got, tt.wantErr)
			}
		})
	}
}

func TestAfter_TableDriven(t *testing.T) {
	t.Parallel()
	floor := time.Date(2025, time.January, 1, 0, 0, 0, 0, time.UTC)
	tests := []struct {
		name    string
		value   time.Time
		wantErr bool
	}{
		{"after floor", floor.Add(time.Second), false},
		{"at floor", floor, true},
		{"before floor", floor.Add(-time.Second), true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			v := New()
			v.After("end", tt.value, floor)
			if got := v.HasErrors(); got != tt.wantErr {
				t.Errorf("After(%s, %s): HasErrors() = %v, want %v",
					tt.value.Format(time.RFC3339Nano), floor.Format(time.RFC3339Nano), got, tt.wantErr)
			}
		})
	}
}
...
func TestChainingReturnsSameInstance(t *testing.T) {
	t.Parallel()
	v := New()
	instant := time.Date(2024, time.January, 1, 0, 0, 1, 0, time.UTC)
	got := v.
		Required("a", "val").
		MinLength("b", "val", 1).
		MaxLength("c", "val", 100).
		...
		OptionalUUID("j", "").
		Before("start", instant, instant.Add(time.Second)).
		After("end", instant.Add(2*time.Second), instant).
		Custom(true, "k", "ok")
	if got != v {
