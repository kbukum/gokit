...
// Supports PostgREST/Supabase filter format: field=op.value Accepts page size from: pageSize,
// page_size, per_page, or limit (in priority order).
...
// clamp confines v to the inclusive range [lower, upper].
// The lower bound is passed even when callers currently always supply 1
// so the helper stays generic for future call sites that need a different minimum.
//
