...
// It separates structured output (for programmatic use) from human-readable content (for LLM consumption).
type Result struct {
	// Output is the structured JSON output for programmatic use.
	Output json.RawMessage `json:"output,omitempty"`
	// Content is a human-readable string for LLM consumption. If empty,
	// Output is used as the content string.
...
// If err is non-nil it takes priority and produces an error block. If r is nil and err is nil,
// an empty success block is returned.
...
// JSONResult creates a Result from a JSON-serializable value. Both Output (raw JSON)
// and Content (JSON string) are set.
