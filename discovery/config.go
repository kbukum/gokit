...
type Config struct {
	...
	// Addr is the discovery provider address (e.g. "localhost:8500"). Generic —
	...
	// ProviderOptions holds exotic backend-specific settings (e.g., datacenter, TLS, connection pool for Consul).
	// Generic fields like addr/scheme/token are on Config directly.
...
type RegistrationConfig struct {
	...
	// Required controls startup behavior when registration fails. When true (the default),
	// the service will retry with backoff
	// and ultimately fail to start if registration cannot be completed —
	// appropriate for staging/production where an undiscoverable service is a silent outage.
	// When false, the service logs a warning and continues in degraded mode —
	// convenient for local development.
	Required bool `yaml:"required" mapstructure:"required"`

	// MaxRetries is the maximum number of registration attempts. Defaults to 3.
	// Only meaningful when Required is true.
	MaxRetries int `yaml:"max_retries" mapstructure:"max_retries"`

	// RetryInterval is the initial resilience backoff interval (e.g. "2s").
	// The shared resilience policy applies exponential backoff from this base. Defaults to "2s".
