...
// The returned Sink typically delegates to the original while adding cross-cutting behavior (logging, metrics, tracing, etc.).
type SinkMiddleware[I any] func(Sink[I]) Sink[I]

// ChainSink composes multiple sink middlewares into one. Middlewares are applied in order:
