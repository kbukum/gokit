...
// ErrNilSinkFunc is returned by SinkFunc.Send when the wrapper was constructed with a nil function.
// Sends fail closed with this typed error rather than panicking on the runtime path.
...
// NewSinkFunc creates a Sink from a plain function. A nil fn is tolerated at construction;
// Send then fails closed with ErrNilSinkFunc instead of panicking.
...
// IsAvailable reports whether the sink can accept sends.
// A SinkFunc built with a nil function is never available,
// since Send always fails with [ErrNilSinkFunc].
