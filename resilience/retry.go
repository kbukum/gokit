...
type RetryConfig struct {
	...
	Jitter float64
	// Rand supplies a uniform random float64 in [0.0, 1.0) used to compute jitter. Leave nil for the concurrency-safe, auto-seeded default; inject a seeded source (e.g. rand.New(rand.NewPCG(seed1, seed2)).Float64) to make backoff deterministic under test.
	Rand func() float64
...
// Retry executes a function with retry logic. Returns the result of the function or the last error if all retries fail.
...
// BackoffDelay returns the normalized retry delay for a failed attempt. attempt is one-based and matches the attempt value passed to RetryConfig.OnRetry. If attempt < 1, it is clamped to 1.
