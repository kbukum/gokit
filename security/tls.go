...
// discovery, and other transport layers.
...
// Returns (nil, nil) if no TLS settings are configured (all fields are zero values) —
// callers treat that as "no TLS" rather than a typed error.
//
// this builder;
// making it a sentinel error would force every transport setup site to errors.Is for a non-error condition.
//
