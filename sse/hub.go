...
const (
	...
	// the newest frame is dropped instead of blocking the hub.
...
// Send sends raw data to the client as a generic SSE `message` event (no named event line).
// Returns false if the channel is full.
func (c *Client) Send(data []byte) bool {
	return c.SendFrame(Frame{Data: data})
}

// SendFrame sends a typed SSE frame. When frame.Event is non-empty,
// the browser's named-event listener for that event fires;
// otherwise the default `message` listener fires. The client queue is bounded; when it is full,
// the new frame is dropped and false is returned so a slow client never blocks the hub
...
// so browser EventSource named-event listeners (`source.addEventListener("X", ...)`) fire.
// Leave empty for a generic `message` event.
type Message struct {
	Pattern string
	Event   string
	Data    []byte
...
// Stop signals the hub to shut down. It closes all client connections and causes Run to return.
...
// The event name becomes the SSE `event:` line
// so browser EventSource named-event listeners (`source.addEventListener(event, ...)`) fire.
//
// This is the primary API for global notifications (e.g. "notifications.new", "catalog.pull.completed").
// For per-resource fan-out where only some subscribers should receive the message,
// use BroadcastToPattern with a caller-defined client ID convention.
//
