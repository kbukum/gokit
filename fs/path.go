...
// Sentinel errors returned by the pure path-validation helpers. They are plain sentinel errors (not AppError) so callers can match with errors.Is; the filesystem-touching helpers return typed AppError values instead.
var (
...
// ValidateRelativePath reports whether path is safe to join under a caller-owned root directory, rejecting absolute paths, '..' traversal, and volume names.
...
// NormalizeRelativePath strips '.' components so semantically equal inputs share one canonical value, while rejecting empty, absolute, volume-prefixed, or '..' paths. A path consisting solely of '.' components collapses to ".".
...
// FindInAncestors searches start and each ancestor directory for a regular file named fileName, returning the nearest match. A nested directory's file shadows one higher up. The boolean is false when no ancestor contains the file.
...
// ParentDir returns the parent directory of path, or false when path has no distinct parent (a filesystem root or ".").
...
// splitSegments splits path into its components using the OS separator, normalizing backslashes on Windows via filepath.ToSlash.
