...
func (m *Manager) ImagePull(ctx context.Context, ref string, opts ...ImagePullOption) error {
	...
	m.log.InfoCtx(ctx, "pulling image", map[string]any{"image": ref})
...
// G117 (gosec) flags the marshaled "Password" field as a secret-shaped JSON key.
// That is intentional here —
// the Docker daemon expects an X-Registry-Auth header containing exactly this payload (registry password included).
// The encoded blob is sent only over the local Docker socket / authenticated daemon connection,
// never logged.
