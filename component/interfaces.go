...
type Component interface {
	...
	// Stop gracefully shuts down the component and releases resources. Implementations must drain inflight work within the context deadline.
...
type Description struct {
	// Name is the human-readable display name (e.g., "HTTP Server", "PostgreSQL"). If empty, the component's Name() is used.
	Name string
	// Type categorizes the component: "database", "server", "kafka", "redis", etc.
	Type string
	// Details is a human-readable one-liner shown in the startup summary. Examples: "localhost:5432 pool=25/5", "localhost:6379 db=0 pool=10"
	Details string
