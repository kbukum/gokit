...
// serverConfig is the assembled, validated configuration for a Server. It is
// populated by ServerOption values grouped by concern across the handler files
// and carries the security.Policy consulted by every gated access.
type serverConfig struct {
	title  string
	prefix string

	// policy holds the fail-closed hardening configuration (allow-lists,
	// authorization, size limits, and audit).
...
// serverOptions assembles the SDK ServerOptions from the configured capability
// handlers, overlaying them onto any caller-supplied base options. The SDK
// infers advertised capabilities (tools/prompts/resources) from registered
// features and from the subscribe/roots handlers wired here.
