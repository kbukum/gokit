...
// ServerOption configures Server creation. Options are applied in order onto a serverConfig; granular hardening options write into the embedded security.Policy, so a later WithPolicy replaces the whole policy while later granular options override individual fields.
type ServerOption func(*serverConfig)

// WithPolicy sets the complete fail-closed hardening policy (allow-lists, authorization decider, auditor, and size limits) in one call. Granular options such as WithAllowedTools or WithMaxInputBytes override individual fields when applied after it.
...
// WithServerOptions supplies base SDK server options. Capability handlers configured through other options are overlaid onto a copy of these.
func WithServerOptions(opts *sdkmcp.ServerOptions) ServerOption {
	return func(c *serverConfig) { c.baseServerOpts = opts }
}

// WithAllowedTools restricts the exposed MCP tool set to the named registry tools. When omitted, all registered tools are exposed. The allow-list is enforced both at registration and at call time (fail closed).
func WithAllowedTools(names ...string) ServerOption {
	return func(c *serverConfig) { c.policy.AllowedTools = security.ToSet(names) }
}

// WithAuthzDecider sets the canonical authz decider consulted for every tools/call before execution.
func WithAuthzDecider(decider authz.Decider) ServerOption {
	return func(c *serverConfig) { c.policy.Decider = decider }
}

// WithAuditor sets the observability auditor that records every gated tool, prompt, and resource access outcome.
...
// WithMaxResultBytes rejects MCP tool results whose serialized output exceeds limit.
func WithMaxResultBytes(limit int) ServerOption {
	return func(c *serverConfig) { c.policy.MaxResultBytes = limit }
}

// WithPrompt registers a static MCP prompt served by handler. Access is gated by the prompt allow-list and recorded through the auditor.
...
// WithAllowedPrompts restricts the callable prompt set to the named prompts. When omitted, all registered prompts are exposed.
func WithAllowedPrompts(names ...string) ServerOption {
	return func(c *serverConfig) { c.policy.AllowedPrompts = security.ToSet(names) }
}

// WithResource registers a static MCP resource served by handler. Reads are gated by the resource allow-list and recorded through the auditor.
...
// WithResourceTemplate registers a static MCP resource template served by handler. The SDK owns URI-template matching; access is gated by the resource allow-list keyed on the template URI.
...
// WithAllowedResources restricts readable resources to the named URIs (or template URIs). When omitted, all registered resources are exposed.
...
// WithSubscribeHandler wires resource subscribe/unsubscribe handling. Setting it advertises the resources.subscribe capability.
...
// WithRootsListChangedHandler wires a handler invoked when a connected client notifies that its roots list changed.
...
// WithProgressHandler wires a handler invoked when a client reports progress against a long-running request.
