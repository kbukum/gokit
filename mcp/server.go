...
//
// It returns an error when registry is nil. Tool, prompt, and resource registration,
// the capability allow-list, size limits, authorization,
// and audit are all applied during construction;
// nothing is exposed that the configuration did not explicitly allow.
...
// Connect binds the server to a single transport session and returns it.
// The caller owns the returned session lifecycle and must Close it.
