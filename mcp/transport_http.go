...
// Localhost (loopback) protection is enabled by default and Origin values are validated up front;
// both fail closed.
type StreamableHTTPConfig struct {
	...
	// AllowedOrigins is the exact set of browser Origins permitted via the returned CrossOriginProtection.
	// Each is validated and normalized.
	AllowedOrigins []string
	// DisableLocalhostProtection turns off the SDK's loopback bind guard.
	// Leave false unless the handler is intentionally exposed beyond localhost.
	DisableLocalhostProtection bool
}

// NewStreamableHTTPOptions builds SDK Streamable HTTP options plus a CrossOriginProtection preloaded with the validated allowed origins.
// Apply the protection as middleware via protection.Handler(next);
// loopback protection is left enabled by default.
...
// StreamableHTTPHandler builds a fully hardened HTTP handler for this server:
// the SDK Streamable HTTP handler wrapped with Origin cross-origin protection and,
// when authToken is non-empty, bearer-token authentication. The handler is the outermost wrapper,
// so unauthenticated or cross-origin requests are rejected before reaching the protocol layer.
func (s *Server) StreamableHTTPHandler(cfg StreamableHTTPConfig, authToken string) (http.Handler, error) {
	...
	// Origin/loopback cross-origin protection is the outermost guard,
	// so blocked origins are rejected before the bearer challenge and never observe auth behavior.
