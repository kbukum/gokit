...
// OAICompatStreamFunc opens a Server-Sent Events stream, POSTing the pre-marshaled JSON body to path. The caller owns transport, auth, headers, and retries; the returned reader is closed by [OAICompatPredictStream] when the stream ends.
...
//
// It validates the request up front (a "prompt" Text input and a model name) and returns an error before opening the stream when either is missing, so a bad request fails fast. Once the stream is open it emits canonical [ai.TextDelta] events per chunk, a trailing [ai.UsageDelta] when the server reports usage, and terminates on the "[DONE]" sentinel or reader EOF. Parse and transport failures surface as a terminal [ai.Error] event.
