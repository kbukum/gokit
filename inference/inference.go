...
// general — adapters cover text generation (vLLM, TGI), classification /
// regression (Triton KServe v2), embeddings (BentoML, custom), image /
// audio inference (custom REST), and arbitrary tensor protocols.
//
...
//
// Capabilities advertise lean adapter hints (streaming, batching, tool-calls)
// to consumers and observability without coupling the inference layer to the
// richer permission envelope owned by package tool. Available reports whether
// the adapter is a working backend (true) or a not-yet-live skeleton (false).
...
// PredictRequest carries arbitrary inputs. Inputs is the canonical payload (a
// map of named inputs to typed values supporting tensors, strings, byte blobs,
// and nested structs); Parameters carries adapter-specific tuning
// (max_new_tokens, temperature, top_k, etc.).
...
// PredictResponse is the normalized response. Outputs is the canonical
// payload; Usage is optional token / compute accounting.
...
// Value is a typed serving input/output. Use TextValue, BytesValue,
// TensorValue, and JSONValue to build values; the Kind field is set by the
// constructor.
type Value struct {
