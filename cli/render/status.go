...
// StatusReporter emits one-off status feedback lines for guided, multi-step CLI flows over an injected writer, palette, and glyph set.
//
// Where progress animates ongoing work and prompt reads input, this renders the short, one-shot status lines a flow emits between steps: a green "✓ Detected Go", a "[1/4]" step counter, a section heading, or a warn/error notice. It composes a [theme.Palette] for color and a [theme.Glyphs] set for the leading symbol, so every line honors NO_COLOR, TTY detection, and UTF-8 capability.
...
// NewStatusReporter builds a reporter from an explicit writer, palette, and glyph set. Callers bind the writer to a real stream (typically stderr, the "diagnostics to stderr" convention) while tests pass an in-memory buffer.
