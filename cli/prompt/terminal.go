...
//
// It abstracts how a prompt reads a line of input and writes output —
// decoupled from what a prompt asks —
// so one set of prompt-kind logic drives both real cooked stdio ([LineTerminal])
// and a deterministic test double ([ScriptedTerminal]).
type Terminal interface {
	// ReadLine reads one whole line. The second return value is false at end of input,
	// so callers surface a typed "input closed" error instead of hanging.
...
//
// The user types a whole line and presses Enter; it never enters raw mode, works over pipes,
// and needs no extra dependencies, so it is the default terminal.
// Bind it to real streams with [NewStdioTerminal], or to in-memory buffers with [NewLineTerminal].
