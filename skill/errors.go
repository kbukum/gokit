...
// Sentinel errors for classifiable skill manifest, loading, verification, and registry failures. Policy rejections (invalid manifest, parse/size/UTF-8/path violations, verification denial, duplicate registration) wrap one of these so callers can match with errors.Is; low-level IO failures (missing or unreadable files) are returned as-is.
var (
	...
	// ErrInvalidPackFile marks a pack path that is not an accepted regular file or directory (symlink, escaping path, wrong type).
