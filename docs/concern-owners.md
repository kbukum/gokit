...
| Token/identity validation | `auth` (`TokenValidator`) | per-transport token contracts; reading credentials from the URL query string as the default | injected into transports (server/grpc/connect/sse) via a local structural interface — L5 never imports `auth` (L6); 401 missing/invalid vs 403 not-permitted. `grpc`/`connect`/`sse` are header-only; `server/middleware` adds an opt-in, path-whitelisted, audited query-token fallback (`WithQueryTokenParam`) for endpoints that cannot set headers |
