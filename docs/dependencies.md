...
These are the third-party modules introduced while bringing gokit to rskit parity. Each lives in an opt-in sub-module so the core stays clean.
...
Modules added during the same parity work that introduced **no** new third-party dependency, by design:
