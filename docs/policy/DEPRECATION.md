...
1. A `// Deprecated:` doc comment **on the line directly above the declaration** (this is the form `gopls`, `staticcheck`, `golangci-lint`, and `pkg.go.dev` recognise).
...
- Adding a new field to a struct (only if the struct is **not** required to be literal-constructed by callers — anything with `unkeyed-fields` warnings needs a deprecation).
