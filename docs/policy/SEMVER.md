...
`gokit` is a Go *workspace* with one root module (`github.com/kbukum/gokit`) and 53 sub-modules (e.g. `github.com/kbukum/gokit/storage`, `…/server`, …). All modules are released in lock-step with path-scoped tags. Consumers should pin only the modules they import.
...
Sub-modules may temporarily be at different versions when a focused fix ships (e.g. `storage/v0.3.0-alpha.2` while the rest of the workspace stays at `v0.3.0-alpha.1`). The next root-level release brings everything back into lock-step.

## Pre-release identifiers

Pre-releases use SemVer suffixes: `v0.3.0-alpha.1`, `v0.3.0-beta.1`. Pre-release tags do not require CHANGELOG entries but **must** be reproducible builds (no moving Go-toolchain reference, no floating action SHAs).
