...
- cosign **keyless** (OIDC) signatures and certificates for every artifact, which the same job then **verifies** with `cosign verify-blob` against the workflow's OIDC identity — the release fails if verification fails;
