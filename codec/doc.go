...
//
// A [Codec] encodes and decodes one on-disk/text format (JSON, TOML, …) through a single canonical in-memory model, [Value]. Any package that reads or writes a config file, manifest, or document reuses these codecs instead of re-implementing "bounded read → parse → typed error" per format.
//
// # Value model
//
