...
//
// Objects merge recursively; on a key collision the overlay value wins. When both sides hold an array at the same key, arrayStrategy is consulted with that key to decide [Replace] vs [Concat]. Type mismatches (for example object vs scalar) resolve to the overlay. Neither input is mutated.
