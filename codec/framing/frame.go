...
// Use it to carry one codec-encoded value per frame over any blocking io.Reader/io.Writer transport (a pipe, a socket, a subprocess's stdio).
//
// [WriteValue] / [ReadValue] encode and decode typed values through an injected codec;
// [WriteFrame] / [ReadFrame] move raw payload bytes when the caller owns serialization.
package framing
...
// DefaultMaxFrameBytes is the default maximum accepted payload size for a single frame (16 MiB).
// It is generous enough for large structured payloads yet bounded
...
// WriteFrame writes one length-delimited frame carrying payload. A plain io.Writer suffices;
// flushing any buffered transport is the caller's concern.
//
// It returns a typed error if payload exceeds maxBytes
...
//
// It returns io.EOF (with a nil payload) on a clean end-of-stream observed before any length byte —
// the peer closed the connection between frames. A partial prefix
// or payload is a hard transport error,
// and a length above maxBytes is rejected before any allocation.
...
// readFull fills buf exactly, returning io.EOF only for a clean leading EOF
// and a typed transport error for a truncated prefix.
