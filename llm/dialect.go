...
const (
	...
	// StreamSSE uses Server-Sent Events format. Used by: OpenAI, Anthropic, Azure OpenAI, most cloud providers.
	StreamSSE
)
...
//
// Dialect implementations live in driver packages outside this package (e.g. github.com/kbukum/gokit/llm/providers/openai). Driver packages expose a Register function that callers invoke against an explicit [DialectRegistry] at startup.
type Dialect interface {
	...
	// ParseStreamChunk extracts content (text and/or tool calls) from a single stream data chunk. The returned chunk is llm-internal; callers consume the assembled public StreamEvent values produced by Adapter.Stream/Provider.Stream.
	ParseStreamChunk(data []byte) (streamwire.Chunk, error)
}

// DialectRegistry stores LLM dialects by name.
//
// Registries are explicit, isolated, and thread-safe. Driver packages (for example github.com/kbukum/gokit/llm/providers/openai) expose Register(*DialectRegistry) functions that populate a registry during application startup. Pass the populated registry to [NewWithRegistry] to create an [Adapter] that resolves its dialect from the registry.
...
// Register adds a dialect to the registry. It returns an error on programmer errors (empty name, nil dialect, duplicate name).
