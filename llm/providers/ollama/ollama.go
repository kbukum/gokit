// Package ollama is a first-class LLM provider for Ollama
// (https://ollama.com), which exposes an OpenAI-compatible
// /v1/chat/completions endpoint at http://localhost:11434 by default.
//
// Per locked decision D3, ollama is a first-class provider for naming
// honesty and discoverability, while reusing the OpenAI wire dialect
// at the code level (DRY at code, distinct at API). Future native
// /api/* endpoints (model auto-pull, num_ctx, keep_alive) can be added
...
// Dialect implements [llm.Dialect] for Ollama. It reuses OpenAI's wire
// shape (Ollama exposes an OpenAI-compatible endpoint) but reports its
// own Name() so observability, configuration, and routing remain honest.
type Dialect struct {
	openai.Dialect
}
