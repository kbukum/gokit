...
// RawJSON carries a raw JSON value verbatim through the llm API. It is the typed
// carrier for provider-specific request extensions ([CompletionRequest.Extra]):
// the bytes stay opaque so the public surface is free of any, and each provider
// dialect decodes them at its own wire boundary.
//
// RawJSON round-trips through both JSON and YAML, so a request may be authored in
// either format alongside the rest of [CompletionRequest]; a YAML value is
...
// UnmarshalYAML decodes a YAML node and stores its equivalent JSON encoding, so
// YAML-authored extensions are carried in the same opaque JSON form as JSON input.
...
// MarshalYAML renders the raw JSON as its decoded value so it serializes as a
// native YAML node rather than an opaque byte string. Empty input serializes as
