...
// CompleteStructured sends a prompt expecting JSON and decodes the response into a value of type T. The model output is untrusted: it is decoded into the concrete type T (a typed trust boundary that rejects shape-mismatched JSON) rather than an opaque map, and a decode failure returns the zero T with an error instead of a partially populated value.
