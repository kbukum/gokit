...
type CompletionRequest struct {
	...
	// Extra carries provider-specific request extensions as a raw JSON value that is merged into the outgoing request body.
	// It is [RawJSON] rather than a decoded map so the public API stays free of any:
	// each provider dialect decodes it at its own wire boundary.
	// Like the rest of CompletionRequest it may be authored in JSON or YAML.
...
// streamChunk is an llm-internal streaming accumulation type. Provider dialects emit these chunks;
// the public streaming API exposes StreamEvent values assembled from them.
type streamChunk = streamwire.Chunk

// streamToolCall carries tool call deltas during streaming;
// the final canonical ai.ToolUseBlock is built after all deltas arrive.
