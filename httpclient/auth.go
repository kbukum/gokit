...
//
// Credentials are always transmitted in request headers, never in the URL query string,
// so tokens do not leak into server logs, proxies, or browser history.
// AuthConfig redacts its secret fields in String, GoString, and any fmt verb,
// so logging a Config that embeds it never exposes credentials.
...
// apply applies authentication to an HTTP request. Credentials are always set as request headers —
...
// String returns a redacted, log-safe description of the auth config.
// Secret values (token, password, API key) are never included.
