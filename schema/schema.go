...
//
// The map[string]any representation is a deliberate,
// documented opaque-value exception to the no-any rule: a JSON Schema document is arbitrary JSON
// and must stay format-agnostic
// so it serializes cleanly to any wire format (OpenAI, Anthropic, MCP, etc.).
type JSON = map[string]any

// Generate creates a JSON Schema from a Go type using struct tags.
// The type parameter T should be a struct with json and optional jsonschema tags.
//
