...
// WithDefinitions keeps $defs and uses $ref for nested types
// instead of inlining all definitions. Default is to inline.
func WithDefinitions() Option {
	return func(c *config) { c.inline = false }
}

// WithAdditionalProperties allows additional properties in the
// generated schema. Default is strict (no additional properties).
