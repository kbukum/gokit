...
// ValidationLimits bounds the structural size of untrusted JSON documents (schemas and values) before they are compiled or validated, guarding against resource-exhaustion from adversarial input.
type ValidationLimits struct {
	...
	MaxKeyBytes int
	// MaxTotalStringBytes is the maximum cumulative UTF-8 byte length across all object keys and string values.
	MaxTotalStringBytes int
}

// DefaultLimits returns the default structural limits applied when none are specified. The values mirror the cross-kit defaults.
...
// check enforces the limits against a decoded JSON document (as produced by json.Unmarshal into any), returning a *LimitError on the first violation.
