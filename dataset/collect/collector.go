...
// Collector orchestrates item collection generically over the item type T:
// it streams each [stage.Source] through the configured transforms and pluggable [stage.Validator],
// classifies items real/AI, records stats in a [manifest.Manifest] cache,
// and publishes to the configured targets. Sources are streamed by a bounded worker pool;
// a single main loop owns the manifest, result, and progress, so no target
// or counter is shared across goroutines.
...
// WithValidator sets the per-item validator every item must satisfy (fail-closed).
// A nil validator accepts every item.
...
// Run collects every source through the worker pool,
// publishing each source's items to the targets as they complete, and persists the manifest cache.
// It fails closed: a source, validation,
// or target error aborts the run (the remaining sources are canceled)
// and no failed source's items are published.
// A resumable source that made partial progress is recorded as partial so a later run continues it.
...
// plan resolves each source's cache state, folding cache hits straight into the result
// and returning the work items the pool must stream (fresh or resumed).
...
// dispatch runs the worker pool over work, folding each source event into the manifest, result,
// and targets from this single goroutine. It cancels the remaining work on the first error
// but always drains events and joins every worker before returning.
...
// A failed source aborts the run; a resumable failed source with progress is recorded partial.
// A successful source is published and marked done.
...
// publish streams one source's items to every target in order, recording each target's result.
// Targets are touched only from the main loop, so no target or result is accessed concurrently.
