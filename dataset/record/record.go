...
//
// The any element type is a deliberate, documented exception to the no-any rule: a record field carries heterogeneous JSON leaf data (string, float64, bool, nil, and nested map/slice) whose leaves cannot be given a closed type. It matches [github.com/kbukum/gokit/codec] Value and schema.JSON.
type Value = any

// Record is one dataset row: an ordered set of named JSON field values. Keys are compared in sorted order so serialization is deterministic.
type Record struct {
	fields map[string]Value
}

// New returns a Record over a copy of fields, so later mutation of the caller's map does not affect the record.
...
// Select returns a new record containing only the named columns that are present, preserving their values.
