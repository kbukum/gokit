...
// ReadCSV reads a CSV file (bounded by limits) and returns a pipeline of its records. The first row is the header; each later row must have the same number of columns or reading fails closed.
func ReadCSV(path string, limits payload.Limits) (*stream.Pipeline[Record], error) {
	return readFile(path, limits, ParseCSV)
}

// ReadJSONArray reads a JSON array file (bounded by limits) whose elements are objects and returns a pipeline of its records.
...
// ReadJSONLines reads a newline-delimited JSON file (bounded by limits) whose non-empty lines are objects and returns a pipeline of its records.
...
// readFile performs the bounded read then applies parse, surfacing any parse error to the caller rather than mid-stream.
...
// ParseCSV parses CSV bytes into records, treating the first row as the header. It fails closed on malformed rows and never panics on untrusted input.
...
// ParseJSONArray parses a JSON array of objects into records. It fails closed when the payload is not an array of objects.
...
// ParseJSONLines parses newline-delimited JSON objects into records, skipping blank lines. It fails closed on any malformed or non-object line.
...
// decodeObject decodes a single JSON object into a Record, rejecting any non-object shape.
