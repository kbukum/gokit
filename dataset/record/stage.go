...
// FileSource is a [stage.Source] that reads a tabular file in one of the
// supported [Format]s. Reading is deferred to stream time and bounded by
// limits, so the source composes with the generic collector. Its cache key
// fingerprints the format and path.
...
// Stream reads the file lazily, surfacing any read or parse error on the first
// pull rather than at construction time.
...
// FileTarget is a [stage.Target] that writes records to a file in one of the
// supported [Format]s via an atomic replace, so a partial write never leaves a
// truncated file. It accumulates records across publishes so a multi-source run
// writes every source's records to the single file rather than clobbering it.
...
// Publish appends the records to the target's accumulated set and rewrites the
// file, reporting the total record count now on disk. Publishing is driven from
// the collector's single main loop, so the accumulator is not shared across
// goroutines.
