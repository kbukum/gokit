...
// Filter returns a pipeline that keeps only the records for which pred returns
// true. It reuses the canonical [stream.Filter] operator.
func Filter(p *stream.Pipeline[Record], pred func(Record) bool) *stream.Pipeline[Record] {
	return stream.Filter(p, pred)
}

// SelectColumns returns a pipeline whose records are projected onto the named
// columns (missing columns are dropped), reusing [stream.Map].
