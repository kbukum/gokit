...
// ApplyTransform returns a pipeline that applies t to each item,
// dropping items the transform does not keep and propagating any transform error.
// It reuses [stream.FlatMap] so no parallel streaming stack is introduced.
