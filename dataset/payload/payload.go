...
// Payload is a bounded byte payload that is either held in memory or backed by a file on disk. Payloads over the in-memory cap must be file-backed so a stage never materializes an oversized blob.
type Payload struct {
	data []byte
	path string
}

// FromBytes returns an in-memory payload, rejecting input larger than the configured in-memory cap so callers spill to a file instead.
...
// FromFile returns a file-backed payload referencing path. The file is not read until [Payload.ReadBounded] or [Payload.WriteTo] is called.
...
// ReadBounded returns the payload bytes, enforcing the in-memory cap. A file-backed payload is read through [fs.ReadFileLimit].
...
// WriteTo streams the payload to w without materializing a file-backed payload in memory, returning the number of bytes written.
