...
// DefaultMaxInMemoryBytes is the default cap on a single in-memory payload or bounded file read (8 MiB), mirroring rskit's dataset limits.
...
// Limits bounds the resources a dataset stage may consume. A payload larger than MaxInMemoryBytes must spill to a file rather than be held in memory, and StreamBuffer caps a stage's in-flight buffer to apply backpressure.
type Limits struct {
...
// WithDefaults fills any zero field with its default so callers may pass a partially-populated Limits.
