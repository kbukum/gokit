...
// Register adds a named TokenValidator. It returns an error if name is empty, the validator is nil, or name is already registered. If this is the first successful registration, the validator becomes the default.
//
// Note: prior versions silently overwrote duplicates and returned no error. Callers that relied on this behavior must Unregister or use a fresh Registry.
