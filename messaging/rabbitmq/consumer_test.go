...
func TestConsumerConsumeAcksSuccessfulMessages(t *testing.T) {
	...
	if !errors.Is(err, context.Canceled) { t.Fatalf("Consume = %v", err) }
	if acks.acked != 1 || acks.nacked != 0 { t.Fatalf("acks=%d nacks=%d", acks.acked, acks.nacked) }
	if got.Topic != "orders" || got.Key != "k" || string(got.Value) != "payload" || got.Headers["x"] != "y" { t.Fatalf("message = %#v", got) }
}
...
func TestConsumerNacksHandlerErrorAndMapsConsumeErrors(t *testing.T) {
	...
	if err := c.Consume(context.Background(), func(context.Context, messaging.Message) error { return handlerErr }); !errors.Is(err, handlerErr) { t.Fatalf("handler error = %v", err) }
	if acks.nacked != 1 { t.Fatalf("nacks=%d, want 1", acks.nacked) }

	consumeErr := errors.New("consume failed")
	c = &Consumer{ch: &fakeRabbitChannel{consumeErr: consumeErr}, topic: "orders", queue: "orders"}
	if err := c.Consume(context.Background(), func(context.Context, messaging.Message) error { return nil }); !errors.Is(err, consumeErr) { t.Fatalf("consume error = %v", err) }
	c.closed = true
	if err := c.Consume(context.Background(), func(context.Context, messaging.Message) error { return nil }); !errors.Is(err, messaging.ErrClosed) { t.Fatalf("closed consume error = %v", err) }
}
...
func TestConsumerEnsureChannelAndClose(t *testing.T) {
	...
	if err != nil { t.Fatalf("NewConsumer: %v", err) }
	if c.Topic() != "orders" { t.Fatalf("Topic = %q", c.Topic()) }
	if got, err := c.ensureChannel(); err != nil || got != ch { t.Fatalf("ensureChannel = %v, %v", got, err) }
	if err := c.Close(); err != nil { t.Fatalf("Close: %v", err) }
	if !ch.closed || !conn.closed { t.Fatalf("closed ch=%v conn=%v", ch.closed, conn.closed) }
	if _, err := c.ensureChannel(); !errors.Is(err, messaging.ErrClosed) { t.Fatalf("closed ensure = %v", err) }
}
...
func FuzzRoutingKeyAndQueueName(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, rkPrefix, qPrefix, topic string) {
		...
		if strings.Trim(rkPrefix, ".") == "" && route != topic { t.Fatalf("route = %q, want topic", route) }
		queue := queueName(cfg, topic)
		if strings.Trim(qPrefix, ".") == "" && queue != route { t.Fatalf("queue = %q, want route %q", queue, route) }
	})
}
