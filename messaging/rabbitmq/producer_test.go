...
func (ch *fakeRabbitChannel) ConsumeWithContext(ctx context.Context, _ string, _ string, _ bool, _ bool, _ bool, _ bool, _ amqp.Table) (<-chan amqp.Delivery, error) {
...
func TestProducerPublishMethodsDeclareQueueAndPublish(t *testing.T) {
	...
	if len(ch.published) != 4 {
		t.Fatalf("published %d messages, want 4", len(ch.published))
	}
	if got := ch.published[0].key; got != "rk.orders" {
		t.Fatalf("routing key = %q", got)
	}
	if got := ch.published[0].msg.Headers["message-key"]; got != "json-key" {
		t.Fatalf("message-key = %v", got)
	}
	var decoded map[string]string
	if err := json.Unmarshal(ch.published[0].msg.Body, &decoded); err != nil || decoded["id"] != "1" {
		t.Fatalf("json payload = %#v err %v", decoded, err)
	}
	if len(ch.declaredQueues) != 3 {
		t.Fatalf("declared queues = %v, want one per topic", ch.declaredQueues)
	}
	if got := ch.published[2].msg.Headers["event-id"]; got != "evt-1" {
		t.Fatalf("event-id = %v", got)
	}
	if got := ch.published[3].msg.Headers["x"]; got != "y" {
		t.Fatalf("send header = %v", got)
	}
}
