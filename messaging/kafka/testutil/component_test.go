...
func TestComponent_Interfaces(t *testing.T) {
	...
	var _ messaging.ProducerCloser = comp.MockProducerClient()
	var _ messaging.Producer = comp.MockProducerClient()
}
...
func TestMockProducer_Publish(t *testing.T) {
	p := &MockProducer{}
	ctx := context.Background()
	event, err := messaging.NewEvent("user.created", "test-service", map[string]string{"id": "123"}, "user-123")
	if err != nil {
...
func TestMockProducer_Publish_WithExplicitKey(t *testing.T) {
	p := &MockProducer{}
	event, err := messaging.NewEvent("test", "src", struct{}{}) // no subject
	if err != nil {
		t.Fatal(err)
	}
...
func TestMockProducer_ClosedErrors(t *testing.T) {
	...
	if err := p.Publish(context.Background(), "t", messaging.Event{}, "k"); err == nil {
		t.Error("expected error on closed producer")
	}
	if err := p.Send(context.Background(), messaging.Message{}); err == nil {
		t.Error("expected error on closed producer")
	}
}
