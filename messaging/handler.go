...
// HandlerMiddleware transforms a MessageHandler by wrapping it with
// additional behavior (logging, metrics, retry, tracing, etc.).
type HandlerMiddleware func(MessageHandler) MessageHandler

// ChainHandlers composes middlewares around a base handler.
// Middlewares are applied so that the first element in the slice is the
