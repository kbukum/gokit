...
// this function only applies the timeout to the stream establishment phase.
// Once the stream is successfully opened,
// it runs with the original context (no timeout affecting the stream lifetime).
//
...
// TryOpenStream attempts to open a stream with a short timeout. If it fails,
// it returns the zero value of T and the error without blocking the caller.
