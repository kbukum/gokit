...
type ToolUseBlock struct {
	...
	//
	// It is deliberately json.RawMessage rather than a decoded map:
	// the arguments are untrusted model output
	// and must be schema-validated (package schema / tool) before being decoded into a typed input.
	// Keeping the bytes opaque avoids a lossy any round-trip
	// and lets each tool decode into its own concrete type at the trust boundary.
...
// substituting an empty object for nil, empty, or JSON null input.
// Any other value is returned trimmed and unchanged — coercion to an object is not attempted,
// since a non-object payload is rejected later by schema validation at the tool trust boundary.
// Providers and stream assemblers use it
// so downstream consumers never have to special-case absent arguments.
