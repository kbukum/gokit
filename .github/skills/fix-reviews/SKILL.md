...
- **Valid?** Judge it against gokit's baseline
...
developer-friendly message that states the change as it stands, **no `Co-authored-by` trailer** and
no review/plan/batch narration. Group commits by pattern when it aids the reader (one commit per
class of fix), or a single tidy commit when the sweep is small.
...
Every fix must still satisfy gokit's engineering baseline
