...
gokit is a multi-module repo where **each module needs its own semver git tag** — without proper tags Go assigns broken pseudo-versions. `tag-modules.sh` tags every module consistently. Full details live in `docs/RELEASING.md`, `docs/VERSIONING.md`, `policy/SEMVER.md`, and `policy/DEPRECATION.md`.
...
A release is the one time to run the **complete** gates rather than the affected set. Everything must be green before tagging:
...
Use `policy/SEMVER.md`. While in `0.x`: a breaking change in the `[Unreleased]` CHANGELOG section bumps **MINOR**; otherwise **PATCH**.
...
`tag-modules.sh` refuses to tag if `[Unreleased]` is the only populated section, or if the `[vX.Y.Z]` section for the version you're cutting is missing.
...
Equivalent Makefile wrappers: `make tag VERSION=vX.Y.Z`, `make tag-push VERSION=vX.Y.Z`, `make list-tags`.

## Step 5 — Publish

Follow the remaining steps in `docs/RELEASING.md` (GitHub release, notes from the CHANGELOG section, SBOM/provenance if configured). CI actions must be SHA-pinned; artifacts signed.
