...
- No data races: shared state guarded by `sync.Mutex`/`sync.RWMutex` or confined to one goroutine; verified under `-race`. Context is the first parameter, never stored in a struct.
