...
L1  config, logging, validation,        L6  auth*, authz, database, cache, storage,
...
L4  bootstrap, stream, chain, dag,      L9  workload, cli, bench, dataset
...
- **Package cohesion & file-level structuring.** A package whose non-test code is piled into one **oversized or concern-mixed file** should be split into concern-named sibling files (`client.go`, `options.go`, `types.go`, `errors.go`, `doc.go`), or a large area promoted to a sub-package. Reorg is **not only** about one oversized file: when a single package/folder has accumulated **more than ~10 non-test files** (excluding `_test.go`) that fall into **2–3+ separable concern groups**, each cohesive group should move into its own concern-named sub-package (sub-folder with its own declare-only `doc.go`), so the reader lands in the right file *and folder* from the layout (as in `agent/{memory}`, `cli/{theme,render,…}`). This is **criteria-driven, not a line count**: reorganize only where the split genuinely improves readability, maintainability, scalability, extendability, and reusability without causing other issues (import cycles, over-fragmentation), or where a package already holds many organizable siblings. A cohesive, single-concern file (or a flat set of files that all serve one concern) is fine at any length — length alone is never the verdict, concern-mixing is; do not split for the sake of it. When a change touches such a file, restructure it **in the same change** rather than postponing the reorg (postponed reorg is how structure debt accumulates). The large-file guard (`scripts/check-structure.sh`, `GOD_FILE_LINES` default 350, counting code lines only) surfaces candidates; an oversized, concern-mixed file left un-split is a should-fix. All of `make structure` is advisory (never gating).
...
- **Naming & organization convention.** Package/file/identifier naming and file/folder organization follow [`docs/naming-and-structure.md`](../../../../docs/naming-and-structure.md): lowercase/singular/non-stuttering packages, call-site-readable exported identifiers, concern-named files, and consistent `testutil` helper naming. A misleading or stuttering name, a catch-all file (`util.go`/`misc.go`) that mixes concerns, or an inconsistent test-helper name is a should-fix.
- **Test placement.** Tests follow the three tiers in [`docs/TEST-LAYOUT.md`](../../../../docs/TEST-LAYOUT.md): tier 1 unit tests co-located in-package (`foo_test.go`), tier 2 module tests black-box (`package foo_test`), tier 3 integration tests in the adapter sub-module that owns the backend (guarded so `go test ./...` stays hermetic). File naming is `foo.go` → `foo_test.go` with no `_edge`/`_extra`/`_coverage` coverage-padding suffixes; shared fakes/clocks/harnesses live in the owning `testutil`, never hand-rolled inline. A coverage-padding test-file name, or a fake duplicated inline that belongs in `testutil`, is a should-fix. `make structure` reports padding-suffix names (advisory).
