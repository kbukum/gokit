...
The canonical owner set is documented in [`docs/concern-owners.md`](../../../../docs/concern-owners.md)
— start there, then check each low-level operation against it. Do not eyeball it
...
- **Filesystem / paths / file IO** → `fs` (path confinement, symlink-escape rejection, atomic writes, permissions)
  and `util` (copy, ensure-dir, read/write helpers).
  Raw `os.Open`+`filepath.EvalSymlinks`+`Rel` escape checks, `os.WriteFile` where atomicity matters,
...
- **Resilience** → `resilience` (retry / timeout / circuit-break), not hand-rolled loops
  or scattered `context.WithTimeout` + custom backoff.
...
The list above is illustrative, not exhaustive: the rule is *any* module,
so if a hit maps to a gokit owner not named here, it still counts.

**3. Judge each candidate — reuse, enhance, add, or justify:**

- Owner covers it → **reuse**: delete the fork, call the owner. *(blocker)*
- Owner is close but inadequate → **enhance it generically** so every consumer benefits, then reuse
  — never fork a tweaked copy.
  "Almost the same" (a near-copy with one changed line, or a copied comment) is still a fork.
- No owner and the capability is genuinely foundational
  and generally useful → **add it to the owning module (or a new one)**, not locally —
  a local solution is a **should-fix** with an "upstream to the owner" note.
