...
- **Tests ship with the change.** New/changed behavior has tests in the same change set; a bug fix has a regression test that fails without the fix. Behavior added with no test is a **blocker**.
- **Behavioral, not implementation-coupled.** Tests assert observable behavior and public contracts, not private field values or call sequences that would break on a harmless refactor.
...
- **Coverage gates.** ≥90% per package (blocking floor), ≥95% aspirational target across all packages; the security-load-bearing packages (`errors`, `auth`, `authz`, `security`, `resilience`, `encryption`) are held to the same ≥90% floor. A change that drops a gated package below its floor is a blocker. (CI enforces project + patch 90% via `codecov.yml`; the per-package floors are checked each step via `make test-coverage`.)
- **Fuzz where it matters.** Parsers, validators, auth/JWT, codecs, and schema have `Fuzz` targets. A new parser/validator with no fuzz target is a should-fix.
- **Environment-independent.** Tests use `t.Setenv` (auto-restored) rather than mutating global env; no ordering dependency between tests.
