...
- **Placement & layering.** Right module (root vs sub-module vs nested adapter); acyclic imports (lower layers never import higher); new packages get `doc.go`.
...
- **Readable files & injected composition.** Split by concern into focused files; injected registries/config; no `init()` side effects or mutable package globals.
