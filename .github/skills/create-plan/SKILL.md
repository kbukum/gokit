...
- **Structure & placement.** Correct module (root vs sub-module vs nested adapter); acyclic layering (lower layers never import higher); every new package has `doc.go`.
...
- **Readable files.** Split by concern into focused files — never pile into one file. The aggregator is declare-only: `doc.go` carries package docs only, never code.
