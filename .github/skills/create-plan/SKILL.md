...
kebab-case (`tmp/typed-di-cycle-detection/`, `tmp/storage-gcs-backend/`) — the same high-level
...
- **`NN-topic.md`** step files (when the work is multi-step) — zero-padded and ordered by
  ...
  files — split into step files as soon as the work is iterative or spans layers.
...
A plan may **not** invent a lighter standard than gokit's. Its cross-cutting rules restate — and
