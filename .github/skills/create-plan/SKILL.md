...
- **Readable files.** Split by concern into focused files — never pile into one file. The aggregator is declare-only: `doc.go` carries package docs only, never code. Scope any structure reorg **together with the change that touches it, per domain** — when a step reshapes signatures or otherwise edits an oversized / concern-mixed file, that same step splits it into concern-named siblings. When a single folder has accumulated many loosely-related non-test files (roughly >5–10) that fall into separable groups, lift each cohesive group into its own concern-named sub-package (sub-folder + declare-only `doc.go`). Reorg is criteria-driven (piled-up or concern-mixed file, or a folder with many loosely-related siblings), never a standalone deferred sweep and never arbitrary.
