...
func createRemote(t *testing.T, repoDir string) string {
	...
	// Align remote HEAD with the source branch so go-git can resolve it during Clone.
	// Use --git-dir from the source repo dir to avoid safe.bareRepository restrictions.
...
func statusShort(t *testing.T, dir string, paths ...string) string {
	t.Helper()
	args := append([]string{"status", "--short", "--"}, paths...)
	return stringTrimSpace(runGit(t, dir, args...))
}
