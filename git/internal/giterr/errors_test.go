...
func TestConstructorsReturnTypedAppErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "SigningKeyMissing", err: SigningKeyMissing("user.signingkey"), code: errors.ErrCodeInvalidInput, status: http.StatusUnprocessableEntity, message: "user.signingkey", detailKey: DetailGitError, detailWant: DetailSigningKeyMissing},
		{name: "IdentityMissing", err: IdentityMissing("user.email"), code: errors.ErrCodeInvalidInput, status: http.StatusUnprocessableEntity, message: "user.email", detailKey: DetailGitError, detailWant: DetailIdentityMissing},
		{name: "FileTooLarge", err: FileTooLarge("README.md", "HEAD", 10, 5), code: errors.ErrCodeInvalidInput, status: http.StatusUnprocessableEntity, message: "exceeding limit", detailKey: DetailGitError, detailWant: DetailFileTooLarge},
		{name: "RemoteAuth", err: RemoteAuth("denied"), code: errors.ErrCodeUnauthorized, status: http.StatusUnauthorized, message: "authentication failed", detailKey: DetailGitError, detailWant: DetailRemoteAuth},
		{name: "PushRejected", err: PushRejected("refs/heads/main", "non-fast-forward"), code: errors.ErrCodeConflict, status: http.StatusConflict, message: "remote rejected", detailKey: DetailGitError, detailWant: DetailPushRejected},
